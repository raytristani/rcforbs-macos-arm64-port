...
///
/// On iOS, libspeex must be bundled as a framework within the app.
class SpeexDecoder {
    ...
    private func loadLibSpeex() {
        // On iOS, look for a bundled framework first, then fall back to system paths
        let bundlePath = Bundle.main.privateFrameworksPath.map { "\($0)/libspeex.framework/libspeex" }
        let paths = [
            bundlePath,
            Bundle.main.path(forResource: "libspeex", ofType: "dylib"),
            // Homebrew paths for Catalyst / simulator builds
